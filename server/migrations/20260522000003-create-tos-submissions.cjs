...
/** Creates tos_submissions (period- + department-scoped). Defensive: skips if sync already made it. */
...
module.exports = {
  async up(queryInterface, Sequelize) {
    if (await tableExists(queryInterface, 'tos_submissions')) return;
    await queryInterface.createTable('tos_submissions', {
  ...
  async down(queryInterface) {
    await queryInterface.dropTable('tos_submissions');
  },
};
